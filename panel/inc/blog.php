...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM blog WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE blog SET baslik = ?, sef = ?, kisa_aciklama = ?, aciklama = ?, img = ? WHERE id = ?");
        $islem = $islem->execute(array($_POST['baslik'],'',$_POST['kisa_aciklama'],$_POST['aciklama'],$_POST['img1'],$_GET['duzenle_id']));
        $id = $_GET['duzenle_id'];
    }else{
        $islem = $db->prepare("INSERT INTO blog SET baslik = ?, sef = ?, kisa_aciklama = ?, aciklama = ?, img = ?");
        $islem = $islem->execute(array($_POST['baslik'],'',$_POST['kisa_aciklama'],$_POST['aciklama'],$_POST['img1']));
        $id = $db->lastInsertId();
    }

    if($islem){
    	$sef = sef($_POST['baslik']).'-'.$id;
    	$islem = $db->prepare("UPDATE blog SET sef = ? WHERE id = ?");
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM blog WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Blog</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
<div class="row">
					                        ...
					                        foreach( $query as $row ){
					                          echo '
					                            <tr>
					                              ...
					                              <td>'.$row['baslik'].'</td>
					                              <td><a href="'.$site.'blog/'.$row['sef'].'">Görmek için tıklayın.</a></td>
					                              <td>
