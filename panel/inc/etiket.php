...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM etiket WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE etiket SET baslik = ?, link = ?, sira = ? WHERE id = ?");
        $islem = $islem->execute(array($_POST['baslik'],$_POST['link'],$_POST['sira'],$_GET['duzenle_id']));
    }else{
        $islem = $db->prepare("INSERT INTO etiket SET  baslik = ?, link = ?, sira = ?");
        $islem = $islem->execute(array($_POST['baslik'],$_POST['link'],$_POST['sira']));
    }
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM etiket WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
}
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Etiket</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
