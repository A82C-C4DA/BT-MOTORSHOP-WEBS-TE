...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM sayfa WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE sayfa SET baslik = ?, sef = ?, kisa_aciklama = ?, aciklama = ?, ust_menu = ?, alt_menu = ? WHERE id = ?");
        $islem = $islem->execute(array($_POST['baslik'],'',$_POST['kisa_aciklama'],$_POST['aciklama'],$_POST['ust_menu'],$_POST['alt_menu'],$_GET['duzenle_id']));
        $id = $_GET['duzenle_id'];
    }else{
        $islem = $db->prepare("INSERT INTO sayfa SET baslik = ?, sef = ?, kisa_aciklama = ?, aciklama = ?, ust_menu = ?, alt_menu = ?");
        $islem = $islem->execute(array($_POST['baslik'],'',$_POST['kisa_aciklama'],$_POST['aciklama'],$_POST['ust_menu'],$_POST['alt_menu']));
        $id = $db->lastInsertId();
    }

    if($islem){
    	$sef = sef($_POST['baslik']);
    	$islem = $db->prepare("UPDATE sayfa SET sef = ? WHERE id = ?");
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM sayfa WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">İç Sayfa</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
			...
			<div class="card-body">
				<div class="table-responsive">
					<table id="example1" class="table table-striped table-bordered text-nowrap  mb-0">
						...
						<tbody>
							<?php
		                      $query = $db->query("SELECT * FROM sayfa ORDER BY id DESC", PDO::FETCH_ASSOC);
		                      if($query->rowCount()){
		                        foreach( $query as $row ){
		                          echo '
			                            <tr>
			                              <td>'.$row['id'].'</td>
			                              <td>'.$row['baslik'].'</td>
			                              <td><a href="'.$site.'sayfa/'.$row['sef'].'">Görmek için tıklayın.</a></td>
			                              <td>
			...
			<div class="card-body pt-10">
				...
				<form action="" method="post">
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<textarea class="form-control" name="kisa_aciklama" required=""><?php echo @$duzenle['kisa_aciklama']; ?></textarea>
						</div>
					</div>
					<div class="form-group row">
