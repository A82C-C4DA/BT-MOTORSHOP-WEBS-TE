...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM kampanya WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE kampanya SET alt_ust = ?, img = ?, link = ?, sira = ? WHERE id = ?");
        $islem = $islem->execute(array(0,$_POST['img1'],$_POST['link'],$_POST['sira'],$_GET['duzenle_id']));
    }else{
        $islem = $db->prepare("INSERT INTO kampanya SET alt_ust = ?, img = ?, link = ?, sira = ?");
        $islem = $islem->execute(array(0,$_POST['img1'],$_POST['link'],$_POST['sira']));
    }
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM kampanya WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Kampanyalar 1</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
						...
						<div class="order-table">
							<div class="table-responsive">
								<table id="example1" class="table table-striped table-bordered text-nowrap  mb-0">
									...
									<tbody>
										<?php
					                      $query = $db->query("SELECT * FROM kampanya WHERE alt_ust = 0", PDO::FETCH_ASSOC);
					                      if($query->rowCount()){
		...
		<div class="card">
			<div class="card-body">
				<form action="" method="post">
					...
					<div class="form-group row">
						<label class="col-md-3 form-label">Sıra</label>
						<div class="col-md-9">
							<input type="number" class="form-control" name="sira" value="<?php echo @$duzenle['sira']; ?>" required="">
						</div>
					</div>
