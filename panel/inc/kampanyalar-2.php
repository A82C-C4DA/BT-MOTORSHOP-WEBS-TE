...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        ...
        $islem = $islem->execute(array(1,$_POST['img1'],$_POST['link'],$_POST['sira'],$_GET['duzenle_id']));
    }else{
        $islem = $db->prepare("INSERT INTO kampanya SET alt_ust = ?, img = ?, link = ?, sira = ?");
        $islem = $islem->execute(array(1,$_POST['img1'],$_POST['link'],$_POST['sira']));
    }
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Kampanyalar 2</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
						...
						<div class="order-table">
							<div class="table-responsive">
								<table id="example1" class="table table-striped table-bordered text-nowrap  mb-0">
									...
									<tbody>
										<?php
					                      $query = $db->query("SELECT * FROM kampanya WHERE alt_ust = 1", PDO::FETCH_ASSOC);
					                      if($query->rowCount()){
