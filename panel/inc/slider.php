...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM slider WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE slider SET img = ?, link = ?, sira = ?, aciklama = ? WHERE id = ?");
        $islem = $islem->execute(array($_POST['img1'],$_POST['link'],$_POST['sira'],$_POST['aciklama'],$_GET['duzenle_id']));
    }else{
        $islem = $db->prepare("INSERT INTO slider SET  img = ?, link = ?, sira = ?, aciklama = ?");
        $islem = $islem->execute(array($_POST['img1'],$_POST['link'],$_POST['sira'],$_POST['aciklama']));
    }
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM slider WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Slider</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
			...
			<div class="card-body">
				<div class="table-responsive">
					<table id="example1" class="table table-striped table-bordered text-nowrap  mb-0">
						...
						<tbody>
							<?php
		                      $query = $db->query("SELECT * FROM slider ORDER BY id DESC", PDO::FETCH_ASSOC);
		                      if($query->rowCount()){
		                        foreach( $query as $row ){
		                      	...
		                      	echo '<tr><td colspan="5"><center><h3>Veri Bulunamadı.</h3></center></td></tr>';
		                      }
		                    ?>
						</tbody>
					</table>
			...
			<div class="card-body pt-10">
				<form action="" method="post">
					<div class="form-group row">
						<div class="col-md-12">
							<input type="text" class="form-control" name="aciklama" placeholder="Açıklama" value="<?php echo @$duzenle['aciklama']; ?>" required="">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-md-12">
							<input type="text" class="form-control" name="link" placeholder="Link" value="<?php echo @$duzenle['link']; ?>" required="">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-md-12">
							<input type="number" class="form-control" name="sira" placeholder="Sıra No" value="<?php echo @$duzenle['sira']; ?>" required="">
						</div>
					</div>
					...
					<div class="form-group row">
						<div class="col-md-12"><div class="form-group"><button type="submit" class="btn btn-success">Değişiklikleri Kaydet</button></div></div>
					</div>
				</form>
			</div>
		</div>
