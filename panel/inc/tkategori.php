...
if(isset($_GET['sil_id'])){
    $ids = alt_kategori_bul1($_GET['sil_id']).$_GET['sil_id'];
    $delete = $db->exec("DELETE FROM tkategori WHERE id IN ({$ids})");
    echo b();
}
...
if($_POST AND !isset($_POST['siralama'])){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE tkategori SET baslik = ?, sef = ?, ust_kategori = ?, ust_menu = ?, alt_menu = ?, aciklama = ?, kisa_aciklama = ?, sira = ? WHERE id = ?");
        ...
        $islem = $db->prepare("INSERT INTO tkategori SET baslik = ?, sef = ?, ust_kategori = ?, ust_menu = ?, alt_menu = ?, aciklama = ?, kisa_aciklama = ?, sira = ?");
    ...
    if($islem){
    	...
    	$islem = $db->prepare("UPDATE tkategori SET sef = ? WHERE id = ?");
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM tkategori WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
if($_POST AND isset($_POST['siralama'])){
	...
	foreach($readbleArray as $row){
		$islem = $db->prepare("UPDATE tkategori SET sira = ?, ust_kategori = ? WHERE id = ?");
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Tasarla Kategori</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
...
<div class="row">
	<div class="col-xl-6">
		<div class="card mg-b-20">
			...
			<div class="card-body">
				<?php 
					$cek1 = $db->query("SELECT * FROM tkategori WHERE ust_kategori = 0 ORDER BY sira ASC", PDO::FETCH_ASSOC);
					if($cek1->rowCount()){
						?>
					            ...
					            <ol class="dd-list">
					                ...
					                <?php
							            function alt_eleman($x,$sayfa){
						            	  global $db;
							              $out = '';
							              $cek1 = $db->query("SELECT id, baslik FROM tkategori WHERE ust_kategori = '{$x}' ORDER BY sira ASC ", PDO::FETCH_ASSOC);
										  if($cek1->rowCount()){
						...
						<div class="form-group row">
						  ...
						  <div class="col-sm-9">
							<select class="form-control select2v1" name="ust_kategori" required="">
								<option value="0">Anatkategori</option>
								<?php
