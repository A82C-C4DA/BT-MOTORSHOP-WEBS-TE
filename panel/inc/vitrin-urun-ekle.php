...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM vitrin_urun WHERE urun_id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}

if($_POST AND !isset($_POST['siralama'])){
	
    $islem = $db->prepare("INSERT INTO vitrin_urun SET vitrin_id = ?, urun_id = ?, sira = ?");
...
if($_POST AND isset($_POST['siralama'])){
	...
	foreach($readbleArray as $row){
		$islem = $db->prepare("UPDATE vitrin_urun SET sira = ? WHERE id = ?");
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Vitrin Ürün</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
		...
		<div class="card">
			...
			<div class="card-body">
				<div class="row">
					<div class="col-md-12">
						<?php 
							$cek = $db->query("SELECT * FROM vitrin_urun WHERE vitrin_id = '{$_GET['vitrin_id']}' ORDER BY sira ASC", PDO::FETCH_ASSOC);
							if($cek->rowCount()){
								?>
