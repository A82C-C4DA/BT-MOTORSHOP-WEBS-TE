...
if(isset($_GET['sil_id'])){
    $delete = $db->exec("DELETE FROM vitrin WHERE id = '{$_GET['sil_id']}' LIMIT 1");
    echo b();
}
...
if($_POST AND !isset($_POST['siralama'])){
    ...
    if(isset($_GET['duzenle_id'])){
        $islem = $db->prepare("UPDATE vitrin SET baslik = ?, img = ?, link = ? WHERE id = ?");
        $islem = $islem->execute(array($_POST['baslik'],$_POST['img1'],$_POST['link'],$_GET['duzenle_id']));
    }else{
        $islem = $db->prepare("INSERT INTO vitrin SET baslik = ?, img = ?, sira = ?, link = ?");
...
if($_POST AND isset($_POST['siralama'])){
	...
	foreach($readbleArray as $row){
		$islem = $db->prepare("UPDATE vitrin SET sira = ? WHERE id = ?");
...
if(isset($_GET['duzenle_id'])){
    $duzenle = $db->query("SELECT * FROM vitrin WHERE id = '{$_GET['duzenle_id']}' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    ?>
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">Vitrin</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Ekle - Düzenle</span>
		</div>
	</div>
</div>
		...
		<div class="card">
			...
			<div class="card-body">
				<div class="row">
					<div class="col-md-12">
						<?php 
							$cek = $db->query("SELECT * FROM vitrin ORDER BY sira ASC", PDO::FETCH_ASSOC);
							if($cek->rowCount()){
								?>
										        ...
										        foreach ($cek as $ak) {
									              echo '<li data-id="'.$ak['id'].'" class="dd-item">
									                      <div class="dd-handle">
					                    					...
					                    					<a href="vitrin-urun-ekle/'.$ak['id'].'" class="duzenle" style="float:right;color:green;margin-right:100px">Ürün Ekle</a>
									                    </li>';
									            }
									        ?>
							            </ol>
